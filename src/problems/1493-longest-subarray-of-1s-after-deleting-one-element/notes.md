...
- Idea: maintain a window with at most one zero (the one we delete). Expand `right`, track the zero count, and pull `left` forward while zeros exceed one. Each valid window represents a candidate streak after deleting one zero, so `windowLength - 1` is the number of ones after the deletion.
...
- Edge: uneven zeros to verify window shrinks properly.
