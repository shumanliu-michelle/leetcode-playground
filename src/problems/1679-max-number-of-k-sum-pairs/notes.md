...
### 1) Hash map counting (implemented)
- Idea: for each number `x`, check if `k - x` is available from previous unmatched values. If available, consume it and increment count; otherwise store `x`.
- Time: O(n)
- Space: O(n)

### 2) Sort + two pointers (alternative)
- Idea: sort ascending. Use `left` and `right` pointers. If sum is too small, increase `left`; if too large, decrease `right`. If sum equals `k`, count one operation and move both pointers.
- Time: O(n log n)
