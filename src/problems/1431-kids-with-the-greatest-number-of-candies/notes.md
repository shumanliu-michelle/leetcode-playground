...
- Idea:
  - First pass: compute `maxCandies`.
  - Second pass: for each kid, check `candies[i] + extraCandies >= maxCandies`.
- Time: `O(n)`
...
- Your initial thought was correct and efficient: compute current max once, then check each kid against it.
