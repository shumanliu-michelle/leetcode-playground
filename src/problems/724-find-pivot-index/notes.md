...
## Solutions
### Prefix-sum `pivotIndex(nums: number[]): number`
- Summary: compute `total = sum(nums)` once, then scan and maintain `leftSum`. At each index the right sum is `total - leftSum - nums[i]`; if it matches `leftSum`, return the index. Otherwise, add `nums[i]` to `leftSum` and continue.
