...
- Idea: `right` scans and consumes one full run of same characters. `left` writes compressed output for that run. Write character always; write digits of count only when count > 1.
- Time: O(n)
- Space: O(1) auxiliary
...
- Run invariant: before each outer iteration, `right` points to next unprocessed run start.
