...
### 1) Map-based (implemented)
- Idea: build frequency maps for both words, sort the keys to ensure the alphabets match, then sort the values to compare frequency multisets.
...
### 2) Optimized with fixed-size arrays (alternative)
- Idea: use fixed-size frequency arrays for 26 lowercase letters, compare presence via `freq[i] === 0`, and compare sorted frequency lists for final validation.
