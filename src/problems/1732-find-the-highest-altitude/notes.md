...
## Solutions
### Prefix-sum `largestAltitude(gain: number[]): number`
- Summary: track the running sum; since the starting-altitude is `0`, initialize `max` to `0`. Update `max` whenever the prefix sum exceeds it.
