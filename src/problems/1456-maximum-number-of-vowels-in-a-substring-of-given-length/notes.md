...
## Solutions
### Sliding-window `maxVowels(s: string, k: number): number`
- Summary: keep a running vowel count for the current window, add the incoming character, subtract the outgoing one (if it was a vowel), and track the highest count seen.
- Time: O(n) — each character is examined exactly twice (entering and leaving the window).
- Space: O(1) — only a constant-size `Set` and scalar counters are required.
